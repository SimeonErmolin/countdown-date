...
function countdown() {
    ...
    const userDate = new Date(_viewJs.UI.INPUT.value);
    if (userDate == 'Invalid Date') return;
    const currentDate = new Date();
    const dateDifference = new Date(userDate - currentDate);
    let result = "After ";
    if (dateDifference.getFullYear() - 1970) result += `${_dateFns.format(dateDifference, 'y') - 1970} years, `;
    if (dateDifference.getDate()) result += `${_dateFns.format(dateDifference, 'd')} days, `;
    if (dateDifference.getHours() - 6) result += `${_dateFns.format(dateDifference, 'H') - 6} hours`;
    _viewJs.UI.RESULT.innerHTML = result;
}
_viewJs.UI.BTN.addEventListener('click', countdown);
